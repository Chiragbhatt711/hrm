...
class EmployeeController extends Controller
{
    ...
    public function cardAllEmployee(Request $request)
    {
        ...
        $users = DB::table('users')
                    ...
                    ->get();
        $userList = DB::table('users')->where('users.admin_id',$admin_id)->get();
    ...
    {
        $admin_id = adminId();
        $users = DB::table('users')->where('users.admin_id',$admin_id)
                    ->join('employees', 'users.user_id', '=', 'employees.employee_id')
                    ->select('users.*', 'employees.birth_date', 'employees.gender', 'employees.company')
                    ->get();
        $userList = DB::table('users')->where('users.admin_id',$admin_id)->get();
    ...
    public function updateRecord( Request $request)
    {
        ...
        try{
            ...
            for($i=0;$i<count($request->id_permission);$i++)
            {
                ...
                module_permission::updateOrCreate(
                    ['id' => $request->id_permission[$i]], // The criteria to find the record
                    $UpdateModule_permissions // The data to update or create
                );
            }
